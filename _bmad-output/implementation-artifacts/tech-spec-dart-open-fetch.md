...
- [x] Task 10: Implement `$ref` resolver
  ...
  - Action: Resolve `$ref` pointers: local (`#/components/schemas/Pet`) and relative file (`./models.yaml#/Pet`). Remote URL `$ref`s (`https://...`) are NOT resolved in v0.1 — log `Diagnostic` with the ref path and skip. For 3.1 compliance, perform full document parse before resolution. For relative file refs, call the injected `FileReader` callback provided by the CLI layer — this keeps core free of `dart:io`.
  - **Circular reference strategy:** Detect cycles via a visited-set during resolution. When a cycle is detected (A→B→A), the resolver:
    1. Creates a placeholder `ApiSchema` with `isCircularRef: true` and a `refTarget` string pointing to the target schema name
    2. After all non-circular refs are resolved, back-fills circular placeholders with direct references to the already-resolved `ApiSchema` objects
    3. IR classes use a `late final` field for the circular ref target, initialized in the back-fill pass. This preserves practical immutability (set once, never changed) while allowing the two-pass resolution.
    4. The code generator detects `isCircularRef` schemas and generates the field type normally — Dart classes can reference each other without issue. `fromJson` uses lazy construction to break infinite recursion.
  - Notes: **Highest complexity task in the entire project.** Handle: refs-to-refs, circular refs (A→B→A), cross-file refs via `FileReader` callback, relative path resolution against `basePath`. Test extensively against Stripe spec. The `FileReader` callback is required in the `OpenApiParser` constructor — CLI provides the implementation.
